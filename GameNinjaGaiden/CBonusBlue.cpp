...
CBonusBlue::CBonusBlue(int id) : CGameObject(id)
{
	this->_width = BONUSBLUE_COLLISION_WIDTH;
	this->_height = BONUSBLUE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1001));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::BONUSBLUE;
}

CBonusBlue::CBonusBlue(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = BONUSBLUE_COLLISION_WIDTH;
	this->_height = BONUSBLUE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1001));
...
CBonusBlue::~CBonusBlue() {}

void CBonusBlue::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
RectEx CBonusBlue::GetBoundingBox()
{
	RectEx rect(
...
void CBonusBlue::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CBonusBlue::SetState(int state)
{
