...
CFireWheel::CFireWheel(int id) : CGameObject(id)
{
	this->_width = FIREWHEEL_COLLISION_WIDTH;
	this->_height = FIREWHEEL_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1002));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::FIREWHEEL;
}

CFireWheel::CFireWheel(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = FIREWHEEL_COLLISION_WIDTH;
	this->_height = FIREWHEEL_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1002));
...
CFireWheel::~CFireWheel(){}

void CFireWheel::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
RectEx CFireWheel::GetBoundingBox()
{
	RectEx rect(
...
void CFireWheel::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CFireWheel::SetState(int state)
{
