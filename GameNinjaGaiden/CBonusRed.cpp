...
CBonusRed::CBonusRed(int id) : CGameObject(id)
{
	this->_width = BONUSRED_COLLISION_WIDTH;
	this->_height = BONUSRED_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1000));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::BONUSRED;
}

CBonusRed::CBonusRed(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = BONUSRED_COLLISION_WIDTH;
	this->_height = BONUSRED_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1000));
...
void CBonusRed::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
RectEx CBonusRed::GetBoundingBox() 
{
	RectEx rect(
...
void CBonusRed::GetBoundingBox(float& left, float& top, float& right, float& bottom) 
{
	left = _positionX;
	bottom = _positionY;
...
void CBonusRed::SetState(int state)
{
