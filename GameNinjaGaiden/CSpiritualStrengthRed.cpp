...
CSpiritualStrengthRed::CSpiritualStrengthRed(int id) : CGameObject(id)
{
	this->_width = SPIRITUALSTRENGTHRED_COLLISION_WIDTH;
	this->_height = SPIRITUALSTRENGTHRED_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1005));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::SPIRITUALSTRENGTHRED;
}

CSpiritualStrengthRed::CSpiritualStrengthRed(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = SPIRITUALSTRENGTHRED_COLLISION_WIDTH;
	this->_height = SPIRITUALSTRENGTHRED_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1005));
...
CSpiritualStrengthRed::~CSpiritualStrengthRed(){}

void CSpiritualStrengthRed::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
void CSpiritualStrengthRed::Render()
{
...
RectEx CSpiritualStrengthRed::GetBoundingBox()
{
	RectEx rect(
...
void CSpiritualStrengthRed::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CSpiritualStrengthRed::SetState(int state)
{
