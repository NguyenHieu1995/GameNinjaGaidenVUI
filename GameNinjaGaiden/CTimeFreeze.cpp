...
CTimeFreeze::CTimeFreeze(int id) : CGameObject(id)
{
	this->_width = TIMEFREEZE_COLLISION_WIDTH;
	this->_height = TIMEFREEZE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1007));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::TIMEFREEZE;
}

CTimeFreeze::CTimeFreeze(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = TIMEFREEZE_COLLISION_WIDTH;
	this->_height = TIMEFREEZE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1007));
...
CTimeFreeze::~CTimeFreeze()
{
}

void CTimeFreeze::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
RectEx CTimeFreeze::GetBoundingBox()
{
	RectEx rect(
...
void CTimeFreeze::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CTimeFreeze::SetState(int state)
{
