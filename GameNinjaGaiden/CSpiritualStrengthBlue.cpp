...
CSpiritualStrengthBlue::CSpiritualStrengthBlue(int id) : CGameObject(id)
{
	this->_width = SPIRITUALSTRENGTHBLUE_COLLISION_WIDTH;
	this->_height = SPIRITUALSTRENGTHBLUE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1004));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::SPIRITUALSTRENGTHBLUE;
}

CSpiritualStrengthBlue::CSpiritualStrengthBlue(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = SPIRITUALSTRENGTHBLUE_COLLISION_WIDTH;
	this->_height = SPIRITUALSTRENGTHBLUE_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1004));
...
CSpiritualStrengthBlue::~CSpiritualStrengthBlue(){}

void CSpiritualStrengthBlue::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
void CSpiritualStrengthBlue::Render()
{
...
RectEx CSpiritualStrengthBlue::GetBoundingBox()
{
	RectEx rect(
...
void CSpiritualStrengthBlue::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CSpiritualStrengthBlue::SetState(int state)
{
