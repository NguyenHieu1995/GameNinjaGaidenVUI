...
CThrowingStart::CThrowingStart(int id) : CGameObject(id)
{
	this->_width = THROWINGSTART_COLLISION_WIDTH;
	this->_height = THROWINGSTART_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1006));

	SetState(eStateItem::HIDEITEM);

	this->_tag = eObjectType::THROWINGSTAR;
}


CThrowingStart::CThrowingStart(int id, int x, int y) : CGameObject(id, x, y)
{
	this->_width = THROWINGSTART_COLLISION_WIDTH;
	this->_height = THROWINGSTART_COLLISION_HEIGHT;
	_animation.push_back(CAnimations::GetInstance()->Get(1006));
...
CThrowingStart::~CThrowingStart()
{
}

void CThrowingStart::Update(DWORD dt, vector<LPCGAMEOBJECT>* coObject)
{
...
RectEx CThrowingStart::GetBoundingBox()
{
	RectEx rect(
...
void CThrowingStart::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = _positionX;
	bottom = _positionY;
...
void CThrowingStart::SetState(int state)
{
